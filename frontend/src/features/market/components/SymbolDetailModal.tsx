...
import { getFundDetail, getMarketHistory } from "../../../api/market";
...
export default function SymbolDetailModal({
  ...
  useEffect(() => {
    ...
    const detailPromise =
      type === "FUND" ? getFundDetail(symbol) : Promise.resolve(null);

    const historyPromise = getMarketHistory(symbol, type, start, end);

    Promise.all([detailPromise, historyPromise])
      .then(([detailRes, historyRes]) => {
        if (cancelled) return;
        setFundDetail(detailRes);
